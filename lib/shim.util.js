...
exports.dfsTraverse = function(tree, callback) {
  var nodes = (Array.isArray(tree) ? tree : [tree]),
      // all the initial nodes have a "undefined" parent
      parents = nodes.map(function() { return undefined; }),
      item, parent;
  ...
  while(nodes.length > 0) {
    ...
    if(item.children) {
      if(Array.isArray(item.children)) {
        nodes = item.children.concat(nodes);
        parents = item.children.map(function() { return item; }).concat(parents);
      } else {
        nodes.unshift(item.children);
