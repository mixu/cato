...
Shim.prototype.prop = function(name, value) {
  throw new Error('Shim.prototype.prop is not implemented on Node!');
};

Shim.prototype.attr = function(name, value) {
  var tag = this.get(this.expr);
...
Shim.prototype.remove = function() {
  var el = this.get(this.expr),
      view = viewById[this.expr],
      self = this;

  // console.log(this.expr , '.remove();', view.parent.children);
  // delete from the parent
  if(view && view.parent && view.parent.children) {
    view.parent.children = view.parent.children.filter(function(item) {
      return item !== view;
    });
    view.parent._renderCache.children = view.parent._renderCache.children.filter(function(item) {
      return item !== view;
    });
  }
  // delete from lookups
  delete viewById[this.expr];
  delete parentTagById[this.expr];
  delete tagById[this.expr];
};
    ...
    if(tag.attribs) {
      ...
      Object.keys(tag.attribs).forEach(function(key) {
        ...
        if(typeof fn == 'function') {
          ...
          // if there are params, take each of those and permanently replace it with the fixed value
          tag.attribs[key] = fn.apply(view || parentView, paramNames.map(function(key) { return safeGet(m, key); }));
          // and add a .listenTo(target, event, fn) to the current view || parent view of the tag
          if(v && m) {
