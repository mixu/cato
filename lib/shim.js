...
function convertTagValueBindings(item, parent) {
  ...
  if(typeof item == 'function') {
    ...
    if(!paramNames) {
      ...
      // console.log(parent, paramNames, parent.model);
  ...
  // or if the item has an attribute that is a function (= attribute binding on the item)
  if(false) {
    // parse the function - retrieve the count and names of the parameters
    // if there are no params, then just run the function once - it never changes
    // if there are params, take each of those and:
    // - add a .listenDom(selector, fn)
  }
  return item;
