...
function convertTagValueBindings(item) {
  // taking advantage of previous parent-child relationships, since this occurs after ".render()" calls have
  // been realized
  ...
  if(typeof item == 'function') {
    ...
    var paramNames = getParamNames(item);
    if(!paramNames) {
  ...
  // and/or if the item has an attribute that is a function (= attribute binding on the item)
  console.log(item);
  if(item.attribs) {
    // check all attribs
...
Shim._attach = function(tree, task) {
  ...
  ShimUtil.dfsTraverse(tree, function(item, parentTag, parentView) {
