...
//  console.log('listenTo', target, eventName, callback);
  return this;
};
...
View.prototype.listenDom = function(selector, callback) {
  ...
  if(typeof jQuery != 'undefined') {
    if (selector === '') {
      ...
      console.log('listenDom', "jQuery(#"+ this.id+").on("+eventName+", callback)");
    } else {
      jQuery(selector).on(eventName, callback);
      console.log('listenDom', "jQuery("+ selector+").on("+eventName+", callback)");
    }
  }
  return this;
};
